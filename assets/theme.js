...
document.addEventListener('DOMContentLoaded', function() {
  ...
  sections.register('redesigned-pdp-template', theme.Product);
...
//Throttled header resizing. Used to account for mobile menu height.
let resizeTimer;
const hamburgerMenu = document.querySelector('.js-mobile-nav-toggle');

function headerFiller() {
  let h = document.querySelectorAll('.header-element')[0].offsetHeight;
  let filler = document.querySelectorAll('.header-filler')[0];
...
window.addEventListener('resize', function() {
    ...
    resizeTimer = setTimeout(headerFiller, 250);
})
...
let scrollDetection = () => {
  ...
  window.addEventListener('scroll', () => {
    ...
    if (scroll > 0) {
      body.classList.add('body--is-scrolling');
    } else {
      body.classList.remove('body--is-scrolling');
    }
  })
} 
...
function hideSiblingMenus(el) {
  Array.prototype.filter.call(el.parentNode.children, child => {
    if (child !== el && child.classList.contains('site-nav--active-dropdown')) {
      child.classList.remove('site-nav--active-dropdown');
    }
  });
}

function showDesktopMenu() {
  let menuItems = document.querySelectorAll('li.site-nav--has-dropdown');
  let menuTimer;
    ...
    menuItems.forEach(menuItem => {
      menuItem.addEventListener('mouseenter', event => {
        ...
        hideSiblingMenus(currentItem);
        clearTimeout(menuTimer);
        currentItem.classList.add('site-nav--active-dropdown');
      })
      ...
      menuItem.addEventListener('mouseleave', event => {
        ...
        menuTimer = setTimeout(() => {
          currentItem.classList.remove('site-nav--active-dropdown');
        }, 300);
      })
  })
}
...
$('document').ready(function() {
  $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-option').addClass('boost-pfs-filter-option-collapsed');
  $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-option-content').addClass('toggled');
  ...
  //   $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-option').addClass('boost-pfs-filter-option-collapsed');
  //   $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-option-content').addClass('toggled');
  //   $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-tree-v .boost-pfs-filter-option-title-heading').attr('aria-expanded' , false);
  //   $('.boost-pfs-filter-tree-v .boost-pfs-filter-option-title-heading').attr('aria-expanded' , false);
  //   $('.boost-pfs-filter-tree-v .boost-pfs-filter-option-title-heading').click();
  // }, 3000);
});
...
//  $(function() {
//     setTimeout(function(){
//       $('.boost-pfs-filter-tree-v .boost-pfs-filter-option-title-heading').attr('aria-expanded' , true);
//       $('.boost-pfs-filter-mobile-style1 .boost-pfs-filter-option').click();
//    }, 3000);
//  });

var cip = $(".video1, .video2").hover( hoverVideo, hideVideo );
